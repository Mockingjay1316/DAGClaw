...
**Project structure**: `core/` (shared orchestration engine), `cli/` (CLI entry point + terminal display), `backend/` (Express + WebSocket server).
...
**Custom stages**: Users define stages in `claw.config.json` or `claw.config.ts`. Loaded by `core/configLoader.ts`, merged with built-in stages, and passed as a registry to the orchestrator.
...
- `core/configLoader.ts` — Custom stage loading from `claw.config.json`/`.ts`, stage merging
...
- `core/runLogger.ts` — Persistent logging to `.claw/runs/`
- `core/memoryManager.ts` — Memory distillation and injection from `.claw/memory/`
